...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (action === 'approveWithdrawal' && txId) {
      ...
      const amountBnb = parseEther(parseFloat(withdrawal.net_amount).toFixed(8));
      const tx = await contract.processWithdrawal(bscAddress, amountBnb, txId.toString());
      const receipt = await tx.wait();
    ...
    if (action === 'manualSend' && to && amount) {
      ...
      const amountBnb = parseEther(parseFloat(amount).toFixed(8));
      const tx = await contract.processWithdrawal(to, amountBnb, 'manual-admin');
      const receipt = await tx.wait();
      return NextResponse.json({ success: true, txHash: receipt.hash });
    }
