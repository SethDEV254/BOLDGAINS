...
function Toast({ msg, onDone }: { msg: string; onDone: () => void }) {
  ...
  return (
    <div className="fixed top-6 right-6 z-50 flex items-center gap-3 px-5 py-3 rounded-xl font-semibold text-sm max-w-sm"
      style={{ background: isErr ? 'rgba(239,68,68,0.15)' : 'rgba(16,185,129,0.15)', border: `1px solid ${isErr ? 'rgba(239,68,68,0.4)' : 'rgba(16,185,129,0.4)'}`, color: isErr ? '#f87171' : '#34d399' }}>
      {isErr ? <AlertTriangle className="w-4 h-4 flex-shrink-0" /> : <CheckCircle className="w-4 h-4 flex-shrink-0" />}
...
export default function ContractPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    const d = await fetch('/api/admin/contract').then(r => r.json());
    if (d.error === 'Forbidden') { window.location.href = '/login'; return; }
    setInfo(d);
    setLoading(false);
  }, []);
  ...
  async function call(action: string, extra?: object) {
    setWorking(action);
    try {
      const r = await fetch('/api/admin/contract', {
    ...
    <div className="max-w-5xl space-y-6">
      ...
      <div className="glass rounded-2xl p-6 gold-border-glow">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          ...
          {/* Pause / Unpause */}
          {info.paused ? (
            <button onClick={() => call('unpause')}
              disabled={!info.hasOperator || !!working}
          ...
          {!confirmEmergency ? (
            <button onClick={() => setConfirmEmergency(true)}
              disabled={!info.hasOperator || !!working || parseFloat(info.balance) === 0}
              className="flex items-center justify-center gap-2 py-3.5 rounded-xl font-bold text-sm disabled:opacity-40 transition-all"
            ...
            {info.pendingWithdrawals.map((t: any) => (
              <div key={t.id} className="flex items-center gap-4 p-4 rounded-xl flex-wrap"
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {t.reference && /^0x[0-9a-fA-F]{40}$/.test(t.reference) && (
                    <p className="font-mono text-amber-500/70 text-xs mt-0.5 truncate">→ {t.reference}</p>
                  )}
                </div>
                ...
                <div className="flex gap-2 flex-shrink-0">
                  <button onClick={() => call('approveWithdrawal', { txId: t.id })}
                    disabled={!info.hasOperator || !!working}
                  ...
                  <button onClick={() => call('rejectWithdrawal', { txId: t.id })}
                    disabled={!!working}
