...
export default function TeamViewSettingsPage() {
  const [ownerAddress, setOwnerAddress] = useState('');

  useEffect(() => {
    fetch('/api/team/contract').then(r => r.json()).then(d => {
      if (d.operatorAddress) setOwnerAddress(d.operatorAddress);
    }).catch(() => {});
  }, []);
    ...
    <div className="max-w-4xl space-y-6">
      ...
      <div className="glass rounded-2xl p-6 gold-border-glow">
        ...
        <div className="space-y-3">
          ...
          <CopyField label="Operator Wallet" value={ownerAddress} />
        </div>
      ...
      <div className="glass rounded-2xl p-6 gold-border-glow">
        ...
        <div className="mt-4 p-3 rounded-xl flex items-start gap-2"
          ...
          <p className="text-xs text-red-400">Admin access is gated by email and password, configured via environment variables. Signing in with the registered admin email grants access.</p>
        </div>
      </div>
